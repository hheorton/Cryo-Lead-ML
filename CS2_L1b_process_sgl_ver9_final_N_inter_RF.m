...
%=========================================================================
 ...
 devs_fline = gradient(gradient(fline(fv10(1):end)));
  devs_fline = gradient(gradient(fline(1:end)));
% test = fline(fv10(1:end));
% test2 = findpeaks(test);
...
%  plot(1:length(fline),fline(maxbin(1)),'r-','linewidth',1.5);
%  plot(1:length(fline),tl,'g-','linewidth',1.5);
...
a_surf_elev = surf_elev(:);
 ...
 sm_surf_elev = zeros(length(a_surf_elev)-1,1);
   for ll = 2:1:(length(a_surf_elev)-1);
    sm_surf_elev(ll,:) = nanmean(a_surf_elev(ll-1:ll+1));
   end

a_surf_elev1 = zeros(length(a_surf_elev),1);
a_surf_elev1(1,1) = nan; 
a_surf_elev1(2,1) = nan;
a_surf_elev1(2:end,:)= sm_surf_elev(1:end);
% a_surf_elev = a_surf_elev1;
%==========================================================================
%                 lead detection [Ricker, 2014]
%==========================================================================

a_pp = pp(:); a_pp(isnan(a_surf_elev1) ==1) = nan;

std = squeeze(CS.SAR.beam_param(1,:,:)); std = std(:);
kur = squeeze(CS.SAR.beam_param(5,:,:)); kur = kur(:);
skew = squeeze(CS.SAR.beam_param(4,:,:)); skew = skew(:);
std(isnan(a_surf_elev1) ==1) = nan;
kur(isnan(a_surf_elev1) ==1) = nan;
skew(isnan(a_surf_elev1) ==1) = nan;
msig = CS2.MEA.backsc_sig_20Hz(:);
msig(isnan(a_surf_elev1) ==1) = nan;
...
type = textread(list_txt(list_l).name,'%s');
...
ssha = a_surf_elev1(type == 1) - mssh(type ==1);
end
if length(lead_x) == 3
    ssha = a_surf_elev1(lead_x) - mssh(lead_x);
   pre_ssha = mean(ssha);
   radar_fb = a_surf_elev1 - (mssh + pre_ssha);
 cs2_sar_lon(cs2_sar_lon == 0) = nan; cs2_lon = cs2_sar_lon(:);
cs2_sar_lat(cs2_sar_lat == 0) = nan;  cs2_lat = cs2_sar_lat(:);
else
if length(lead_x) == 1
    pre_ssha = a_surf_elev1(lead_x) - mssh(lead_x);
    radar_fb = a_surf_elev1 - (mssh+pre_ssha);
 cs2_sar_lon(cs2_sar_lon == 0) = nan; cs2_lon = cs2_sar_lon(:);
cs2_sar_lat(cs2_sar_lat == 0) = nan;  cs2_lat = cs2_sar_lat(:); 
else
...
if isempty(lead_x) == 1;
    ssha = [];
    radar_fb = a_surf_elev1 - (mssh+nan);
 cs2_sar_lon(cs2_sar_lon == 0) = nan; cs2_lon = cs2_sar_lon(:);
cs2_sar_lat(cs2_sar_lat == 0) = nan;  cs2_lat = cs2_sar_lat(:);
else
if length(lead_x) >= 2
lead_ssha1 = zeros(size(a_surf_elev1));
lead_ssha1(:) = nan;
lead_ssha1(type == 1) = ((a_surf_elev1(type == 1)) - (mssh(type == 1)));
inter_x = find(type == 1);
...
if isempty(inter_find1) == 1
    inter_find1 = nan;
end
inter_idx1 = inter_find1(1);
inter_idx_end = inter_find1(end);
if isnan(inter_idx1) ==1
        inter_idx1 = 1;
end 
if isnan(inter_idx_end) == 1;
        inter_idx_end = length(l); 
end
...
radar_fb = a_surf_elev1 - (mssh+ssha);
...
radar_fb(radar_fb > 1) = nan;
  ...
  radar_fb1 = zeros(length(a_surf_elev)-1,1);
   for ll = 2:1:(length(a_surf_elev)-1);
       radar_fb1(1,1)=nan;
    radar_fb1(ll,:) = nanmean(radar_fb((ll-1:ll+1)));
   end
   radar_fb = radar_fb1;
...
ncwriteatt([filename '.nc'],'surf','valid_max',max(a_surf_elev1));
ncwriteatt([filename '.nc'],'surf','valid_min',min(a_surf_elev1));
...
ncwrite([filename '.nc'],'surf',a_surf_elev1);
